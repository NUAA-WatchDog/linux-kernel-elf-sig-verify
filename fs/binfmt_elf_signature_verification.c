...
enum {Enabled, Magic};
#define ELF_SV_FMT_PRESERVE_ARGV0 (1 << 31)
#define ELF_SV_FMT_OPEN_BINARY (1 << 30)
#define ELF_SV_FMT_CREDENTIALS (1 << 29)
#define ELF_SV_FMT_OPEN_FILE (1 << 28)
 ...
 * locking is done in load_elf_signature_verification_binary
 */
...
static int load_elf_signature_verification_binary(struct linux_binprm *bprm)
{
    ...
    printk("exec elf_signature_verification\n");
...
static char *check_special_flags(char *sfs, Node *e)
{
	...
	while (cont) {
		...
		case 'P':
			...
			p++;
			e->flags |= ELF_SV_FMT_PRESERVE_ARGV0;
			break;
		case 'O':
			pr_debug("register: flag: O (open binary)\n");
			p++;
			e->flags |= ELF_SV_FMT_OPEN_BINARY;
			break;
		case 'C':
			...
			/* this flags also implies the
			   open-binary flag */
			e->flags |= (ELF_SV_FMT_CREDENTIALS |
					ELF_SV_FMT_OPEN_BINARY);
			break;
		case 'F':
			pr_debug("register: flag: F: open interpreter file now\n");
			p++;
			e->flags |= ELF_SV_FMT_OPEN_FILE;
			break;
		default:
			cont = 0;
		}
...
/*
 * Set status of entry/binfmt_elf_signature_verification:
 * '1' enables, '0' disables and '-1' clears entry/binfmt_elf_signature_verification
 */
...
static void entry_status(Node *e, char *page)
{
	...
	dp += sprintf(dp, "flags: ");
	if (e->flags & ELF_SV_FMT_PRESERVE_ARGV0)
		*dp++ = 'P';
	if (e->flags & ELF_SV_FMT_OPEN_BINARY)
		*dp++ = 'O';
	if (e->flags & ELF_SV_FMT_CREDENTIALS)
		*dp++ = 'C';
	if (e->flags & ELF_SV_FMT_OPEN_FILE)
		*dp++ = 'F';
	*dp++ = '\n';
...
static void bm_evict_inode(struct inode *inode)
{
	...
	if (e && e->flags & ELF_SV_FMT_OPEN_FILE)
